...
GO_LIVE_LOCATION = "http://127.0.0.1:9003"  # change host/port when deployed
...
def send_driver_location_to_live_service(driver_id, lat, lng, ts=None, meta=None):
    ...
    try:
        ...
        # Service not running - this is expected in development
        if current_app:
            current_app.logger.debug("Live location service not available on port 9000: %s", e)
        else:
            print("Live service not running:", e)
        return None
